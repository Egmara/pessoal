...
function SSolveUpFact(c,A,b)
  ...
  x, base = Simplex_Rev_comUpFact(ca,B,b,collect(n+1:n+m),collect(1:n),xb)
  ...
  x, base, nbase = Simplex_Rev_comUpFact(c,A,b,base,nbase,x[base])
  return x, base, nbase, dot(c,x)
end

function Simplex_Rev_comUpFact(c,A,b,base,nbase,xb)
  ...
  LU = lufact(A[:,base])
  L0 = LU[:L]; U = LU[:U]; P0 = LU[:p]; Q0 = LU[:q]; v = LU[:Rs]
  x[base] = xb
  #println(x)
